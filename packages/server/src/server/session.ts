...
export class Session {
  ...
  // Extracted from the constructor to keep its cyclomatic complexity within limit.
  private initializeSshController(): SshSessionController | null {
    const sshManager = this.sshManager;
    if (!sshManager) {
      return null;
    }
    return createSshSessionController({
      ...
      connectHandler: (connectInput) => {
        const handler = sshManager.connectHandler;
        if (!handler) {
      ...
      forwardStart: (id) =>
        sshManager.forwardStart?.(id) ??
        Promise.resolve({ ok: false, error: "Port forwarding is not available on this host" }),
      forwardStop: (id) => sshManager.forwardStop?.(id) ?? Promise.resolve({ ok: true }),
      uploadRuntime: () => sshManager.uploadRuntime ?? null,
    });
  }
  ...
  constructor(options: SessionOptions) {
    ...
    this.sshController = this.initializeSshController();
  ...
  private async dispatchInboundMessage(msg: SessionInboundMessage, source?: object): Promise<void> {
    const promise =
      ...
      this.dispatchWorkspacePreferencesAndFilesMessage(msg, source) ??
  ...
  private async handleCheckoutGitOpRequest(msg: CheckoutGitOpRequest): Promise<void> {
    ...
    try {
      ...
      if (msg.op !== "diff-paths") {
        await this.gitMutation.notifyGitMutation(cwd, "git-op", { invalidateForge: true });
