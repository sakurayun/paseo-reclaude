...
export function createWorkspaceProvisioningService(deps: {
  ...
  // Fork: opening a plain directory that contains exactly one git repo (up to a
  // few levels deep) targets that repo instead of the parent folder, so
  // parent-directory workspaces land on the actual checkout.
  async function resolveDirectoryTarget(cwd: string): Promise<LocalCheckoutWorkspaceTarget> {
    return resolveLocalCheckoutWorkspaceTarget(resolve(cwd), {
      workspaceGitService,
      scanGitRepos: deps.scanGitRepos,
    });
  }
  ...
  async function findOrCreateWorkspaceForDirectory(cwd: string): Promise<PersistedWorkspaceRecord> {
    const normalizedCwd = (await resolveDirectoryTarget(cwd)).cwd;
    const workspaces = await workspaceRegistry.list();
