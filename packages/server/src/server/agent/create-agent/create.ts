...
async function resolveMcpCreateAgent(
  ...
  return {
    config: buildMcpSessionConfig({
      ...
      resolvedThinkingOptionId: resolvedCreateConfig.thinkingOptionId,
    }),
    createOptions: {
      ...(labels ? { labels } : {}),
      workspaceId: requireResolvedWorkspaceId(workspaceId),
...
async function resolveMcpProviderCreateConfig(params: {
  ...
  return params.dependencies.providerSnapshotManager.resolveCreateConfig({
    ...
    model: params.resolvedProviderModel.model,
    thinkingOptionId: params.input.thinking,
...
function buildMcpSessionConfig(params: {
  ...
  resolvedThinkingOptionId?: string | null;
}): AgentSessionConfig {
  const passthroughConfig = params.input.config;
  ...
  const config: AgentSessionConfig = {
    ...
    thinkingOptionId:
      (params.resolvedThinkingOptionId === undefined
        ? params.input.thinking
        : (params.resolvedThinkingOptionId ?? undefined)) ?? passthroughConfig?.thinkingOptionId,
