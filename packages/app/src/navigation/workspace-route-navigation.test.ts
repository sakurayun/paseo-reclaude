...
// expo-router ships raw .tsx source that this repo's node-environment vitest does not
// transform; the module is only reachable through the default deps (unused here, since
// every test injects its own dismissTo), so a lightweight mock keeps it from loading.
vi.mock("expo-router", () => ({
