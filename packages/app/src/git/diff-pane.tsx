...
// Submodule rows show the moved commit range (fork feature); regular files show
// the additions/deletions stat. Kept as its own component so the branching stays
// out of DiffFileHeader's complexity budget.
function DiffFileHeaderStat({ file }: { file: ParsedDiffFile }) {
  if (file.isSubmodule) {
    return (
      <>
...
export function DiffFileBody({
  ...
  return (
    <View style={FILE_SECTION_BODY_STYLE} onLayout={handleLayout} testID={testID}>
      {(() => {
        if (file.isSubmodule && cwd !== undefined && serverId !== undefined) {
          return (
            <SubmoduleDiffBody
              file={file}
              cwd={cwd}
...
export function GitDiffPane({
  ...
  return (
    <View style={styles.container}>
      <DiffPaneBranchHeader
        visible={branchHeaderVisible}
      ...
      <DiffPaneForgeSetupCallout message={forgeSetupMessage} />

      <DiffPaneActionError message={prErrorMessage} />
...
function DiffPaneBranchHeader({
  ...
  return (
    <View style={styles.header} testID="changes-header">
      ...
      />
      <View style={styles.headerTrailing}>{children}</View>
    </View>
  );
}

function DiffPaneForgeSetupCallout({ message }: { message: string | null }) {
  if (!message) {
    return null;
  }
  return (
...
function DiffPaneActionError({ message }: { message: string | null }) {
  if (!message) {
    return null;
  }
  return <Text style={styles.actionErrorText}>{message}</Text>;
}
