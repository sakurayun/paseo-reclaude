...
Grok-specific ACP behavior stays in the Grok adapter (and small adjacent modules such as `grok-background-tasks.ts` and `grok-session-descriptor.ts`): mode mapping for Always Approve (`bypassPermissions` id + launch `--always-approve`; do not use a blocking ACP `prompt` for `/always-approve` during setMode — that freezes create_agent / draft send), suppression of `user_message_chunk` when `_meta.hideFromScrollback === true`, and structured `_x.ai/session/update` task events rendered as synthetic `tool_call` timeline items. Shared ACP only exposes narrow hooks (`shouldSuppressUserMessageChunk`, `extensionNotificationHandler`) so other providers remain unaffected.

Grok import discovery reads on-disk session indexes under `$GROK_HOME/sessions/<encoded-cwd>/<session-id>/summary.json` (default `~/.grok/sessions`). Grok ACP advertises `loadSession` but does **not** implement `session/list` (`sessionCapabilities` is empty and the method returns Method not found), so the shared ACP listing probe always returns nothing. `GrokACPAgentClient.listImportableSessions` therefore scans `summary.json` for title/prompt previews and last activity, and `importSession` resumes via ACP `session/load` using the native session id as `providerHandleId`, restoring `current_model_id` / `reasoning_effort` when present. Do not add a degraded `session/list` fallback for Grok.
